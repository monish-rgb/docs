...
- **[Production considerations](#production-considerations)**: multi-tenancy, async, and durability
...
- **Use async middleware methods.** Custom [middleware](/oss/langchain/middleware/custom) should implement async hooks (e.g., `abefore_agent` instead of `before_agent`).
- **Use async for external resource lifecycle.** Creating [sandboxes](#sandboxes) or connecting to [MCP servers](/oss/langchain/mcp) involves network calls and should be awaited. This is why [graph factories](/langsmith/graph-rebuild) that provision these resources are async.
